...
pub struct SpaceState {
    ...
    /// When true, FF proofs are generated by a background task instead of inline
    /// in `add_change` (so a client's write is not blocked for ~seconds while the
    /// STARK is produced). Enabled only for live spaces via `get_or_create_space`;
    /// left `false` in tests so proving stays synchronous and deterministic.
    pub async_proving: bool,
    /// Guard: a background FF proof is currently in flight. The recursive proof
    /// extension MUST stay sequential, so at most one runs at a time.
    pub proving: bool,
    /// Wakes the per-space background prover task when a batch becomes due.
    prove_signal: Arc<Notify>,
...
/// Owned inputs captured under the space lock so the slow FF STARK can run
/// off-lock (see [`SpaceState::capture_ff_prove_inputs`]).
struct CapturedFfInputs {
    changelog: ChangeLog,
    change_responses: Vec<ChangeResponse>,
    previous_proof: Option<FFProof>,
    pruned_tree_bytes: Vec<u8>,
    start_idx: usize,
    new_proven_up_to: usize,
    tree_at_tip: Option<merk::Node>,
    space_id: SpaceId,
}
...
async fn ff_prover_loop(weak: Weak<Mutex<SpaceState>>, signal: Arc<Notify>) {
    loop {
        ...
        loop {
            ...
            // ── prove (slow, off-lock, on a blocking thread) ──
            ...
            state.proving = false;
            // A new batch may have accumulated while we proved — chain into it.
            let num = state.changelog.num_changes() as usize;
            if num >= state.changelog.proven_up_to + state.ff_batch_size {
                state.proving = true;
                continue;
            }
            break;
...
pub(crate) async fn get_or_create_space(
    ...
    // Enable background FF proving and start the per-space prover task so a
    // client's write is not blocked for ~seconds while the STARK is produced.
    let prove_signal = {
        let mut g = arc.lock().await;
        g.async_proving = true;
        g.prove_signal.clone()
    };
    tokio::spawn(ff_prover_loop(Arc::downgrade(&arc), prove_signal));
...
impl SpaceState {
    ...
    pub async fn init_server(
        ...
        let mut new_server_state = Self {
            ...
            async_proving: false,
            proving: false,
            prove_signal: Arc::new(Notify::new()),
    ...
    /// Check if a new FF proof is due and, if so, either signal the background
    /// prover (`async_proving`) or generate it inline (sync — tests / non-live
    /// paths). Generating a STARK takes seconds; the async path keeps that off
    /// the caller's write path and off the space lock.
    fn maybe_generate_ff_proof(&mut self) -> Result<(), ServerError> {
        ...
        if num_changes < proven_up_to + self.ff_batch_size {
            return Ok(());
        }
        if self.async_proving {
            // Hand the batch to the per-space background prover. The `proving`
            // guard keeps the recursive extension sequential (one at a time);
            // batches that arrive while a proof is in flight are picked up when
            // the prover re-checks after finishing.
            if !self.proving {
                self.proving = true;
                self.prove_signal.notify_one();
            }
            return Ok(());
        }
        // Synchronous path (deterministic; used by tests and any non-live space).
        self.generate_ff_proof_sync(num_changes)
    }

    /// Inline (blocking) FF proof generation — the original behavior.
    fn generate_ff_proof_sync(&mut self, num_changes: usize) -> Result<(), ServerError> {
        log::info!(
        ...
        let tip = self.db.snapshot();
        self.finalize_ff_proof(tip)
    }

    /// Post-proof bookkeeping shared by the sync + async prover paths: refresh
    /// the in-memory `ff_proof`, record the tree snapshot at the newly-proven
    /// tip, and persist the checkpoint. `changelog.ff_proof` / `proven_up_to`
    /// must already be updated (via `set_ff_proof` / `update_changelog_proof`).
    fn finalize_ff_proof(&mut self, tree_at_tip: Option<merk::Node>) -> Result<(), ServerError> {
        self.ff_proof = Some(FFProof::deserialize(&self.changelog.ff_proof).map_err(|e| {
            ServerError::Generic(format!(
                "space={} failed to deserialize newly generated FF proof: {e}",
                self.space_id
            ))
        })?);
        self.tree_snapshot = Some(tree_at_tip.ok_or_else(|| {
            ServerError::Generic(format!(
                "space={} missing tree snapshot after FF proof update",
                self.space_id
            ))
        })?);
        self.change_store
    ...
    /// Capture everything the off-lock STARK needs. Trace generation (which is
    /// the only part that reads the `merk::Node`) runs here under the lock and
    /// is fast; the returned inputs are owned so the slow `prove_ff_chunk` can
    /// run on a blocking thread without holding the space lock. Returns `None`
    /// when nothing is pending.
    fn capture_ff_prove_inputs(&self) -> Result<Option<CapturedFfInputs>, String> {
        let start_idx = self.changelog.proven_up_to;
        let new_proven_up_to = self.changelog.num_changes() as usize;
        if start_idx >= new_proven_up_to {
            return Ok(None);
        }
        let tree_snapshot = self
            .tree_snapshot
            .as_ref()
            .ok_or_else(|| format!("space={} missing batch-start tree snapshot", self.space_id))?;
        let Some(pruned_tree_bytes) =
            prepare_ff_pruned_tree(&self.changelog, &self.change_responses, tree_snapshot)?
        else {
            return Ok(None);
        };
        Ok(Some(CapturedFfInputs {
            changelog: self.changelog.clone(),
            change_responses: self.change_responses.clone(),
            previous_proof: self.ff_proof.clone(),
            pruned_tree_bytes,
            start_idx,
            new_proven_up_to,
            // Snapshot of the tree at the newly-proven tip (captured now, under
            // the lock, so it matches `new_proven_up_to` even if more changes
            // land while we prove off-lock).
            tree_at_tip: self.db.snapshot(),
